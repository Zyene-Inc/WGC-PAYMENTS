...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      ...
      // Redirect to onboarding application
      router.push("/dashboard/church/onboarding");
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h2 className="text-3xl font-bold text-wgc-navy-900 tracking-tight">Create your onboarding account</h2>
        <p className="mt-3 text-sm text-slate-500 font-medium">
          Start your mission-driven sub-merchant application.
        </p>
      </div>
        ...
        <div className="bg-white py-10 px-8 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border border-slate-200 rounded-[2.5rem] sm:px-12">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 gap-6">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2 font-mono">Full Name</label>
                <input
                  {...register("fullName")}
                  className={cn(
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2 font-mono">Organization Name</label>
                <input
            ...
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Initiate Onboarding"}
            </button>
            ...
            <div className="flex items-center justify-center gap-2 pt-4 border-t border-slate-50">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">Bank-grade security protocol</span>
            </div>
          </form>

          <p className="mt-8 text-center text-xs font-bold text-slate-400 uppercase tracking-[0.1em] font-mono">
            Already have an account?{" "}
            <Link href="/auth/login" className="text-wgc-gold-600 hover:text-black transition-colors">
              Sign In
            </Link>
          </p>
        </div>
